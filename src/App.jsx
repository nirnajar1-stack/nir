...
export default function App() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 md:p-8 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen font-sans selection:bg-indigo-200" dir="rtl">
      ...
      <div className="flex justify-center mb-8">
        <div className="bg-slate-200/60 p-1.5 rounded-2xl flex border border-slate-300/40 shadow-sm flex-wrap justify-center gap-1">
          <button
            type="button"
      ...
      {section === 'analytics' && (
        <div className="mt-10 bg-white p-4 rounded-2xl shadow-md border border-slate-100 flex flex-wrap gap-4 justify-center items-center max-w-4xl mx-auto">
          <span className="text-slate-500 font-medium text-sm border-l border-slate-200 pl-4 ml-2">{LABELS.legend}</span>
          {Object.keys(COLORS).map((key) => (
            <div key={key} className="flex items-center gap-2 px-2 py-1 hover:bg-slate-50 rounded-lg transition-colors cursor-default">
              <div className="w-3 h-3 rounded-full shadow-inner" style={{ backgroundColor: COLORS[key] }} />
